...
namespace ADSD.Crypto
{
    ...
    public class RIPEMD160Managed  : HashAlgorithm
    {
        ...
        {
            return (RIPEMD160Managed) CryptoConfig.CreateFromName(hashName);
        }
        ...
        {
            InitializeState();
            Array.Clear((Array) _blockDWords, 0, _blockDWords.Length);
            Array.Clear((Array) _buffer, 0, _buffer.Length);
        }
        ...
        private unsafe void _HashData(byte[] partIn, int ibStart, int cbSize)
        {
            ...
            fixed (uint* blockDWords = _blockDWords)
            {
                ...
                {
                    Buffer.BlockCopy((Array) partIn, srcOffsetBytes, (Array) _buffer, dstOffsetBytes, 64 - dstOffsetBytes);
                ...
                while (byteCount >= 64)
                {
                    Buffer.BlockCopy((Array) partIn, srcOffsetBytes, (Array) _buffer, 0, 64);
                    srcOffsetBytes += 64;
                    byteCount -= 64;
                    MDTransform(blockDWords, state, block);
                }
                if (byteCount > 0)
                    Buffer.BlockCopy((Array) partIn, srcOffsetBytes, (Array) _buffer, dstOffsetBytes, byteCount);
            }
        }
